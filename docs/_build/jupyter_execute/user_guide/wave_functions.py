...
#
# In abTEM, a focused beam of electrons, or `Probe`, is defined in reciprocal space as
#
# $$
...
# $$
#
# where $\vec{r} = (x, y)$ is the real space coordinate in $x$ and $y$, respectively. The probe wave function is normalized such that its intensity sums to $1$ in reciprocal space
#
# $$
...
probe.build().diffraction_patterns(max_angle=60, parity="odd").show(
    ax=ax1, title="Reciprocal-space probe intensity"
)
...
#
# We can create a `LineProfile` profile from the `Ìmage` as below.
...
waves_focal_series = probe.build()
