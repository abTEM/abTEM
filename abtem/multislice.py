...
def transition_potential_multislice_and_detect(
    ...
    if detectors_elastic is not None:
        # Declared but never read: elastic detectors passed here were accepted
        # in silence and only the inelastic measurement came back, so a caller
        ...
        raise NotImplementedError(
            "detectors_elastic is not implemented: the core-loss multislice "
            "returns only the inelastic measurement. Run a separate "
            "Probe.scan() with the same scan and potential for the elastic "
            "image."
        )
