...
def potential_derivative(r, p):
    dvdr = -(
        ...
        + p[0, 1] * (2.0 / (p[1, 1] * r**2) + 2.0 / r + p[1, 1]) * np.exp(-p[1, 1] * r)
        + p[0, 2] * (2.0 / (p[1, 2] * r**2) + 2.0 / r + p[1, 2]) * np.exp(-p[1, 2] * r)
        + p[0, 3] * (2.0 / (p[1, 3] * r**2) + 2.0 / r + p[1, 3]) * np.exp(-p[1, 3] * r)
        + p[0, 4] * (2.0 / (p[1, 4] * r**2) + 2.0 / r + p[1, 4]) * np.exp(-p[1, 4] * r)
    )
    return dvdr
