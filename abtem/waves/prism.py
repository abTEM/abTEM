...
from abtem.waves.prism_utils import (
    prism_wave_vectors,
    plane_waves,
    wrapped_crop_2d,
    prism_coefficients,
...
class SMatrixArray(HasArray, AbstractSMatrix):
    ...
    def reduce(
        ...
        scan = validate_scan(
            scan, Probe.from_ctf(extent=self.extent, ctf=ctf, energy=self.energy)
        )
...
class SMatrix(AbstractSMatrix):
    ...
    def _s_matrix_partial(self):
        ...
        potential_partial = (
            self.potential.from_partitioned_args()
            if self.potential is not None
            else None
        )
        return partial(
            ...
            **self.copy_kwargs(exclude=("potential",))
        )
    ...
    def multislice(
        ...
        s_matrix = self.__class__(
            potential=potential, **self.copy_kwargs(exclude=("potential",))
        )
