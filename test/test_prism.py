...
def test_prism_matches_probe(data, lazy, device):
    ...
    s_matrix_diffraction_patterns = s_matrix.reduce(lazy=lazy).diffraction_patterns(max_angle=None)
    probe_diffraction_patterns = probe.build(lazy=lazy).diffraction_patterns(max_angle=None)
...
def test_prism_scan(data, interpolation, detector, lazy, device):
    ...
    s_matrix = data.draw(abtem_st.s_matrix(potential=potential, max_interpolation=max_interpolation, device=device))
...
def test_s_matrix_matches_probe_no_interpolation(data, interpolation, detector, lazy, device):
    potential = data.draw(abtem_st.potential(device=device, ensemble_mean=False))
    detector = data.draw(detector())
    s_matrix = data.draw(abtem_st.s_matrix(potential=potential, max_interpolation=1, device=device))
    ...
    scan = GridScan()
    assume((max(detector.angular_limits(probe)) <= max(probe.cutoff_angles)) or isinstance(detector, WavesDetector))
