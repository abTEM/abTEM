...
from abtem import Probe, PlaneWave, SMatrix
...
def test_normalized(grid_data, wave_data, lazy, builder):
    plane_wave = builder(**grid_data, **wave_data)
    wave = plane_wave.build(lazy=lazy)
    wave = wave.compute()
...
def test_s_matrix_empty_multislice(atom_data, wave_data, sampling, lazy, planewave_cutoff, device):
    ...
    plane_wave = SMatrix(planewave_cutoff=planewave_cutoff, sampling=sampling, device=device, **wave_data)
    wave = plane_wave.multislice(atoms, lazy=lazy).reduce().compute()
    if wave_data['normalize']:
        check_is_normalized(wave.array)
...
@pytest.mark.parametrize("lazy", [True, False])
def test_scatter(atoms_data, wave_data, sampling, lazy):
