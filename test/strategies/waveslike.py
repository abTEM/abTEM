...
def waves(draw,
    ...
    energy = draw(core_st.energy())
    return Waves(array=array, sampling=sampling, ensemble_axes_metadata=axes, energy=energy)
...
def s_matrix(draw,
             ...
             min_interpolation=1,
             max_interpolation=1,
             min_planewave_cutoff=5.,
             max_planewave_cutoff=20.,
             downsample=False,
             potential=None,
    ...
    energy = draw(core_st.energy())
    interpolation = draw(st.integers(min_value=min_interpolation, max_value=max_interpolation))
    planewave_cutoff = draw(st.floats(min_value=min_planewave_cutoff, max_value=max_planewave_cutoff))
