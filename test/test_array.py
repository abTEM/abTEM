...
def _make_parity_waves(gpts=12, n_e=2, n_c=3, seed=0):
    import dask.array as da
    import numpy as np
    ...
    rng = np.random.default_rng(seed)
    array = (
        rng.normal(size=(2, n_e, n_c, gpts, gpts))
        + 1j * rng.normal(size=(2, n_e, n_c, gpts, gpts))
    ).astype(np.complex64)
    static = (
    ...
    waves = Waves(
        ...
        ensemble_axes_metadata=[
            ...
            EnergyLossAxis(values=tuple(float(e) for e in range(n_e))),
            FrozenPhononsAxis(_ensemble_mean=False),
        ],
    )
    waves._static_exit_wave = static
    return waves, array, static


@pytest.mark.parametrize("suffix", ["", ".zip"])
def test_to_zarr_from_zarr_roundtrips_static_exit_wave(tmp_path, suffix):
    """Waves.static_exit_wave (see EnergyResolvedAtomsEnsemble's
    parity_projection) is a plain object attribute, not part of the
    constructor/metadata contract -- it must be written and read back
    explicitly by to_zarr/from_zarr rather than silently dropped."""
    import numpy as np

    from abtem.array import from_zarr

    waves, array, static = _make_parity_waves()
    url = str(tmp_path / f"parity_waves{suffix}")
    waves.to_zarr(url)

    loaded = from_zarr(url)
    np.testing.assert_allclose(loaded.array.compute(), array)

    assert loaded.static_exit_wave is not None
    np.testing.assert_allclose(loaded.static_exit_wave.compute(), static)


@pytest.mark.parametrize("suffix", ["", ".zip"])
def test_to_zarr_from_zarr_without_static_exit_wave_is_unaffected(tmp_path, suffix):
    """A Waves object with no static_exit_wave (the ordinary, non-parity
    case) must round-trip exactly as before -- no spurious
    "static_exit_wave0" entry, and .static_exit_wave stays None."""
    import numpy as np
    ...
    array = (
        rng.normal(size=(2, 3, 8, 8)) + 1j * rng.normal(size=(2, 3, 8, 8))
    ).astype(np.complex64)
    waves = Waves(
        array, energy=100e3, sampling=0.1,
        ensemble_axes_metadata=[
            EnergyLossAxis(values=(0.0, 1.0)),
            FrozenPhononsAxis(_ensemble_mean=False),
        ],
    )

    url = str(tmp_path / f"plain_waves{suffix}")
    waves.to_zarr(url)
    loaded = from_zarr(url)

    np.testing.assert_allclose(loaded.array.compute(), array)
    assert loaded.static_exit_wave is None


def test_from_zarr_reads_old_files_without_static_exit_wave(tmp_path):
    """A zarr store written before this feature existed (no
    "static_exit_wave0" array) must still load fine -- static_exit_wave
    simply stays None, not an error."""
    import numpy as np
    import zarr

    from abtem.array import from_zarr

    waves, array, _ = _make_parity_waves()
    url = str(tmp_path / "parity_waves_legacy.zarr")
    waves.to_zarr(url)

    # simulate a pre-feature file by deleting the companion array
    root = zarr.open(url, mode="a")
    del root["static_exit_wave0"]
