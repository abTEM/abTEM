...
@pytest.mark.parametrize('scan',
                         [scan_st.custom_scan(), scan_st.line_scan(), scan_st.grid_scan()],
                         ids=['custom_scan', 'line_scan', 'grid_scan'])
def test_scan_ensemble(data, scan, chunks):
    scan = data.draw(scan)
    blocks = scan._ensemble_blockwise(chunks).compute()

    for i in np.ndindex(blocks.shape):
        blocks[i] = blocks[i].get_positions()

    assert np.allclose(concatenate_blocks(blocks), scan.get_positions())
